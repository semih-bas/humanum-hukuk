...
try {
  ...
  const rejectedInputs = await Promise.all([
    ...
    expectRejectedValidation("installment count below range", { ...validPayload, installmentCount: 0 }),
    expectRejectedValidation("installment count above range", { ...validPayload, installmentCount: 13 }),
    expectRejectedValidation("fractional installment count", { ...validPayload, installmentCount: 1.5 }),
