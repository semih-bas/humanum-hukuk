...
type Props = { currentUser: { id: string; name: string }; notes: NoteDraft[]; setNotes: Dispatch<SetStateAction<NoteDraft[]>>; onBack: () => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void; saving: boolean; error: string };
...
export default function NoteStep({ currentUser, notes, setNotes, onBack, onSubmit, saving, error }: Props) {
  ...
  return <form className={styles.form} onSubmit={onSubmit}>{error && <p className={styles.error}>{error}</p>}
    ...
    <section className={styles.notes}><h2>Notlar ({notes.length})</h2>{notes.length === 0 ? <p>Henüz not eklenmedi. Bu alan zorunlu değildir.</p> : notes.map((note) => <article key={note.clientId}><div className={styles.author}><i>{initials(currentUser.name)}</i><span><b>{currentUser.name}</b><small>Şimdi</small></span></div><header><b>{typeLabel(note.noteType)}</b>{note.important && <em>Önemli</em>}<button type="button" onClick={() => setNotes((current) => current.filter((item) => item.clientId !== note.clientId))}>Kaldır</button></header><p>{note.content}</p><small>{note.visibility === "TEAM" ? "Tüm ekip" : "Sadece ben"}</small></article>)}</section>
