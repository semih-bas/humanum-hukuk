...
export default function GeneralLegalCasesClient() {
  ...
  return <main className={styles.page}>
    <header className={`${styles.intro} ${buttonStyles.header}`}><div><p className={styles.crumb}>Ana Sayfa <span>›</span> Genel Dava ve Arabuluculuk</p><h1>Genel Dava ve Arabuluculuk</h1><p>Dava ve arabuluculuk dosyalarını, duruşmaları ve süreçleri tek ekrandan yönetin.</p></div><div className={styles.headerActions}><Link className={buttonStyles.button} href="/genel-dava-ve-arabuluculuk/yeni">＋ Yeni Dosya</Link></div></header>
...
function DrawerContent({ tab, overview }: { tab: DrawerTab; overview: Overview }) {
  ...
  if (tab === "summary") return <SummaryPanel detail={detail} />;
