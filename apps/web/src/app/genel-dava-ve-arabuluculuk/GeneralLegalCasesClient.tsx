...
function ActionIcon() {
  return <svg viewBox="0 0 24 24" aria-hidden="true"><path d="m4 20 4-1 11-11-3-3L5 16Z" /></svg>;
}
  ...
  return <main className={styles.page}>
    ...
    <section className={styles.workspace}>
      ...
      <div className={styles.tableWrap}><table><thead><tr><th>#</th><th>Dosya No</th><th>Mahkeme / Birim</th><th>Dosya Türü</th><th>Müvekkil / Başvuran</th><th>Karşı Taraf</th><th>Dosya Değeri</th><th>Aşama</th><th>Son İşlem</th><th>Sonraki Duruşma</th><th>Sorumlu</th><th>Durum</th><th>İşlemler</th></tr></thead><tbody>
        ...
        {!loading && result.items.map((item, index) => <tr key={item.id}><td>{(result.pagination.page - 1) * result.pagination.pageSize + index + 1}</td><td><b className={styles.caseNumber}>{item.referenceNumber}</b></td><td>{item.court ?? (item.kind === "MEDIATION" ? "Arabuluculuk Bürosu" : "—")}</td><td>{item.caseType}</td><td>{primaryParty(item.parties, item.kind, true)}</td><td>{primaryParty(item.parties, item.kind, false)}</td><td>{formatMoney(item.caseValue)}</td><td><span className={styles.stage}>{stageLabels[item.stage]}</span></td><td>{item.lastProcess?.action ?? "—"}</td><td className={item.nextHearing ? styles.hearingDate : ""}>{item.nextHearing ? formatDateTime(item.nextHearing.startsAt) : "—"}</td><td>{item.responsibleUser.name}</td><td><span className={`${styles.status} ${styles[`status_${item.status}`]}`}>{statusLabels[item.status]}</span></td><td><div className={styles.rowActions}><Link href={`/genel-dava-ve-arabuluculuk/yeni?edit=${encodeURIComponent(item.id)}`} aria-label={`${item.referenceNumber} dosyasını düzenle`} title="Dosyayı düzenle"><ActionIcon /></Link><DeleteCaseButton endpoint={`/api/general-legal-cases/${item.id}`} reference={item.referenceNumber} onDeleted={() => setReload((value) => value + 1)} /></div></td></tr>)}
