...
function ActionIcon({ name }: { name: "payment" | "bell" | "eye" | "edit" }) {
  const paths = {
...
export default function GeneralLegalCasesClient() {
  const router = useRouter(); const searchParams = useSearchParams(); const linkedCaseId = searchParams.get("case")?.slice(0, 200) ?? "";
  ...
  const [selectedId, setSelectedId] = useState<string | null>(linkedCaseId || null); const [detail, setDetail] = useState<Detail | null>(null); const [overview, setOverview] = useState<Overview | null>(null);
  ...
  useEffect(() => { function closeOnEscape(event: globalThis.KeyboardEvent) { if (event.key !== "Escape") return; setSelectedId(null); setDetail(null); setOverview(null); setEditing(false); if (linkedCaseId) router.replace("/genel-dava-ve-arabuluculuk", { scroll: false }); } window.addEventListener("keydown", closeOnEscape); return () => window.removeEventListener("keydown", closeOnEscape); }, [linkedCaseId, router]);

  function clearFilters() { setQuery(""); setKind("ALL"); setStatus("ALL"); setStage("ALL"); setPage(1); }
  function openDrawer(id: string) { setDrawerTab("summary"); setSelectedId(id); router.replace(`/genel-dava-ve-arabuluculuk?case=${encodeURIComponent(id)}`, { scroll: false }); }
  function closeDrawer() { setSelectedId(null); setDetail(null); setOverview(null); setEditing(false); if (linkedCaseId) router.replace("/genel-dava-ve-arabuluculuk", { scroll: false }); }
  ...
  return <main className={styles.page}>
    ...
    <section className={styles.workspace}>
      ...
      <div className={styles.tableWrap}><table><thead><tr><th>#</th><th>Dosya No</th><th>Mahkeme / Birim</th><th>Dosya Türü</th><th>Müvekkil / Başvuran</th><th>Karşı Taraf</th><th>Dosya Değeri</th><th>Aşama</th><th>Son İşlem</th><th>Sonraki Duruşma</th><th>Sorumlu</th><th>Durum</th><th>İşlemler</th></tr></thead><tbody>
        {loading && <tr><td colSpan={13} className={styles.empty}>Dosyalar yükleniyor…</td></tr>}{!loading && !result.items.length && <tr><td colSpan={13} className={styles.empty}>Henüz kayıtlı dosya bulunmuyor.</td></tr>}
        {!loading && result.items.map((item, index) => <tr key={item.id}><td>{(result.pagination.page - 1) * result.pagination.pageSize + index + 1}</td><td><b className={styles.caseNumber}>{item.referenceNumber}</b></td><td>{item.court ?? (item.kind === "MEDIATION" ? "Arabuluculuk Bürosu" : "—")}</td><td>{item.caseType}</td><td>{primaryParty(item.parties, item.kind, true)}</td><td>{primaryParty(item.parties, item.kind, false)}</td><td>{formatMoney(item.caseValue)}</td><td><span className={styles.stage}>{stageLabels[item.stage]}</span></td><td>{item.lastProcess?.action ?? "—"}</td><td className={item.nextHearing ? styles.hearingDate : ""}>{item.nextHearing ? formatDateTime(item.nextHearing.startsAt) : "—"}</td><td>{item.responsibleUser.name}</td><td><span className={`${styles.status} ${styles[`status_${item.status}`]}`}>{statusLabels[item.status]}</span></td><td><div className={styles.rowActions}><Link className={styles.paymentAction} href={`/genel-dava-ve-arabuluculuk/yeni?edit=${encodeURIComponent(item.id)}&step=2`} aria-label={`${item.referenceNumber} dosyasının tahsilatlarını aç`} title="Tahsilatlar"><ActionIcon name="payment" /></Link><Link className={styles.reminderAction} href={`/genel-dava-ve-arabuluculuk/yeni?edit=${encodeURIComponent(item.id)}&step=5`} aria-label={`${item.referenceNumber} dosyasının bildirimlerini aç`} title="Bildirimler"><ActionIcon name="bell" /></Link><button className={styles.viewAction} type="button" aria-label={`${item.referenceNumber} dosyasını görüntüle`} title="Görüntüle" onClick={() => openDrawer(item.id)}><ActionIcon name="eye" /></button><Link className={styles.editAction} href={`/genel-dava-ve-arabuluculuk/yeni?edit=${encodeURIComponent(item.id)}`} aria-label={`${item.referenceNumber} dosyasını düzenle`} title="Düzenle"><ActionIcon name="edit" /></Link><DeleteCaseButton endpoint={`/api/general-legal-cases/${item.id}`} reference={item.referenceNumber} onDeleted={() => setReload((value) => value + 1)} /></div></td></tr>)}
    ...
    {selectedId && <><button type="button" className={styles.drawerBackdrop} aria-label="Dosya özetini kapat" onClick={closeDrawer} /><aside className={styles.drawer} ref={drawerRef} tabIndex={-1} aria-label="Dosya özeti"><header className={styles.drawerHeader}><div><small>SEÇİLİ DOSYA</small><h2>Dosya Özeti</h2></div><button type="button" onClick={closeDrawer} aria-label="Dosya özetini kapat">×</button></header>
      {drawerLoading ? <div className={styles.drawerState}><span className={styles.spinner} />Dosya bilgileri yükleniyor…</div> : drawerError && !detail ? <div className={styles.drawerState}>{drawerError}</div> : detail && <><div className={styles.drawerIdentity}><div className={styles.identityHeading}><div><strong>{detail.referenceNumber}</strong><p>{detail.court ?? (detail.kind === "MEDIATION" ? "Arabuluculuk" : "Mahkeme bilgisi yok")}</p></div><span className={`${styles.status} ${styles[`status_${detail.status}`]}`}>{statusLabels[detail.status]}</span></div><div className={styles.identityStats}><span><small>Dosya Türü</small><b>{detail.caseType}</b></span><span><small>Dosya Değeri</small><b>{formatMoney(detail.caseValue)}</b></span><span><small>Güncel Aşama</small><b>{stageLabels[detail.stage]}</b></span></div></div>
        {!editing && overview && <><nav className={styles.drawerTabs}>{([["summary","Genel Bilgiler"],["parties","Taraflar"],["hearings","Duruşmalar"],["documents","Evraklar"],["process","İşlemler"],["finance","Tahsilatlar"],["tasks","Bildirimler"],["notes","Notlar"]] as Array<[DrawerTab,string]>).map(([value,label]) => <button type="button" key={value} className={drawerTab === value ? styles.activeTab : ""} onClick={() => setDrawerTab(value)}>{label}</button>)}</nav><div className={styles.drawerBody}><DrawerContent tab={drawerTab} overview={overview} /></div></>}
...
function DrawerContent({ tab, overview }: { tab: DrawerTab; overview: Overview }) {
  ...
  if (tab === "tasks") return <RecordList empty="Kayıtlı bildirim bulunmuyor." items={overview.tasks.map((item) => ({ id: item.id, title: item.title, badge: item.status, description: `${formatDateTime(item.dueAt)} · ${item.assignee?.name ?? "Atanmamış"} · ${item.priority}` }))} />;
