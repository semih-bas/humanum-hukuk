...
export default function NewInsuranceCaseForm({ caseId, initialData, readOnly = false, initialTab = "general" }: { caseId?: string; initialData?: ExistingCase; readOnly?: boolean; initialTab?: InsuranceCaseTab } = {}) {
  const router = useRouter();
  const [form, setForm] = useState(() => makeInitialForm(initialData)); const [tab, setTab] = useState<InsuranceCaseTab>(initialTab); const [files, setFiles] = useState<File[]>([]); const [payments, setPayments] = useState<InsurancePaymentEntry[]>(() => initialData?.payments?.map((payment) => ({ ...payment, clientId: payment.id })) ?? []);
  ...
  return <AppShell pageTitle={readOnly ? "Dosya Genel Bakışı" : caseId ? "Dosyayı Düzenle" : "Yeni Dosya"} pageSubtitle="Sigorta ve Tahkim"><main className={styles.page}><header className={styles.header}><div><Link href="/sigorta-ve-tahkim">←</Link><p>{initialData?.referenceNumber ? `${initialData.referenceNumber} · ` : ""}Sigorta ve tahkim dosya bilgileri.</p></div><div>{caseId && !readOnly ? <button type="button" aria-label="Düzenlemeyi kaydedip kapat" title="Kaydet ve kapat" onClick={() => (document.getElementById("insurance-case-form") as HTMLFormElement | null)?.requestSubmit()}>×</button> : <Link href="/sigorta-ve-tahkim">{readOnly ? "Kapat" : "İptal"}</Link>}{!readOnly && !caseId && <button form="insurance-case-form" disabled={saving}>{saving ? "Kaydediliyor…" : "Kaydet"}</button>}</div></header>
    <nav className={styles.tabs}>{([["general","Genel Bilgiler"],["payments","Ödemeler"],["note","Not"],["notifications","Bildirimler"],["documents",`Belgeler (${(initialData?.documents?.length ?? 0) + files.length})`]] as Array<[InsuranceCaseTab,string]>).map(([key,label]) => <button type="button" key={key} className={tab === key ? styles.activeTab : ""} onClick={() => setTab(key)}>{label}</button>)}</nav>
    {error && <p className={styles.error}>{error}</p>}<form id="insurance-case-form" onSubmit={submit}><fieldset disabled={readOnly} className={styles.formFields}>
      ...
      {tab === "note" && <section className={`${styles.tabPanel} ${styles.descriptionPanel}`}><h2>Not</h2><p>Dosyayla ilgili çalışma notunu yazın.</p><textarea maxLength={500} value={form.description} onChange={(e) => update("description", e.target.value)} placeholder="Notunuzu yazın…" /><b>{form.description.length} / 500</b></section>}
      ...
      {tab === "documents" && <section className={`${styles.tabPanel} ${styles.documentPanel}`}><h2>Belgeler</h2><p>PDF, JPG veya PNG belgeleri ekleyin. Her belge en fazla 20 MB olabilir.</p>{initialData?.documents?.length ? <div className={styles.fileList}>{initialData.documents.map((document) => <div key={document.id}><a href={`/api/insurance-arbitration/${caseId}/documents/${document.id}`}>{document.originalName}</a></div>)}</div> : null}{!readOnly && <label className={styles.dropzone}><input type="file" accept=".pdf,.jpg,.jpeg,.png" multiple onChange={(e) => setFiles((current) => [...current, ...Array.from(e.target.files ?? [])])} /><strong>Belge seçin veya buraya sürükleyin</strong></label>}<div className={styles.fileList}>{files.map((file,index) => <div key={`${file.name}-${index}`}><span>{file.name}</span><small>{Math.ceil(file.size / 1024)} KB</small><button type="button" onClick={() => setFiles((current) => current.filter((_,itemIndex) => itemIndex !== index))}>×</button></div>)}</div></section>}
