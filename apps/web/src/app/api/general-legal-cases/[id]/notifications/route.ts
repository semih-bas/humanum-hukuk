import { NextResponse } from "next/server";
import { ApiRequestError, assertSameOrigin, readJsonBody, requireApiSession } from "@/lib/api-security";
import { addGeneralCaseTask } from "@/lib/general-legal-cases/activity-service";
import { GeneralLegalCaseNotFoundError } from "@/lib/general-legal-cases/read";
import { notificationInputSchema } from "@/lib/notification-input";
import { resourceIdSchema } from "@/lib/resource-id";

export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) { try {
  assertSameOrigin(request); const session = await requireApiSession(request); const id = resourceIdSchema.safeParse((await params).id);
  if (!id.success) throw new ApiRequestError(404, "NOT_FOUND", "Dosya bulunamadı.");
  const input = notificationInputSchema.safeParse(await readJsonBody(request));
  if (!input.success) return json({ error: { message: "Bildirim bilgileri geçerli değil.", fields: input.error.flatten().fieldErrors } }, 400);
  return json({ data: await addGeneralCaseTask(id.data, input.data, { id: session.user.id, role: session.user.role }) }, 201);
} catch (error) { if (error instanceof ApiRequestError) return json({ error: { code: error.code, message: error.message } }, error.status); if (error instanceof GeneralLegalCaseNotFoundError) return json({ error: { code: "NOT_FOUND", message: "Dosya bulunamadı." } }, 404); console.error("Failed to add general case notification", error); return json({ error: { message: "Bildirim eklenemedi." } }, 500); } }
function json(body: unknown, status: number) { return NextResponse.json(body, { status, headers: { "Cache-Control": "no-store" } }); }
