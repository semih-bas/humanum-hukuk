...
const paymentAmount = money.refine((value) => value.gt(0), "Tutar sıfırdan büyük olmalıdır.");
const paymentSchema = z.object({
  type: z.enum(["OUTGOING_PAYMENT", "INSURANCE_INCOME", "ARBITRATION_INCOME", "ENFORCEMENT_INCOME"]),
  paymentDate: date,
  amount: paymentAmount,
  commission: money,
  description: optionalText(500),
}).strict().superRefine((value, context) => {
  if (value.commission.gt(value.amount)) context.addIssue({ code: "custom", path: ["commission"], message: "Komisyon tutardan büyük olamaz." });
});
...
export const insuranceCaseInputSchema = z.object({
  ...
  postageExpense: money, enforcementExpense: money, arbitrationApplicationFee: money, expertFee: money, postalAmount: money, actualDepreciationAmount: money, description: optionalText(500), payments: z.array(paymentSchema).max(250),
}).strict().superRefine((value, context) => {
