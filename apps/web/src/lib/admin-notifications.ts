...
export type NotificationSource = "enforcement" | "insurance";
...
export async function listAdminNotifications() {
  ...
  const [enforcementCount, insuranceCount, reminders, insurance] = await Promise.all([
  ...
  return { items, totalCount: enforcementCount + insuranceCount, generatedAt: now.toISOString() };
}
...
export async function listAdminReminderTasks() {
  ...
  return { totalCount: enforcementCount + insuranceCount, items };
}
